<?php
if (!class_exists('Pragyan_Widget_Base')) {

	abstract class Pragyan_Widget_Base extends WP_Widget
	{
			...
			switch ($field['type']) {
				...
				case "dropdown_categories":
					...
					$cat_default_args = array(
						...
						'show_option_all' => esc_html__('All Categories', 'pragyan'),
						'echo' => false,
						'multiple' => false
					...
					<p>
						...
						if (taxonomy_exists($taxonomy)) {
							...
							echo '<h4>' . sprintf(esc_html__('Taxonomy (%s) not found', 'pragyan'), $taxonomy) . '</h4>';
						}
						$this->description($field) ?>
					</p>
					<?php
				...
				<div class="media-uploader" id="<?php echo $this->get_field_id('background_image'); ?>">
					<div class="custom_media_preview" style="position:relative;">
						...
						<button class="media_upload button"
								...
								data-choose="<?php esc_attr_e('Choose an image', 'pragyan'); ?>"
								data-update="<?php esc_attr_e('Use image', 'pragyan'); ?>"
								style="width:100%;margin-top:6px;margin-right:30px;"><?php esc_html_e('Select an Image', 'pragyan'); ?></button>
					</div>
				...
				case "icon-picker":
					...
					<div class="ae-icon-picker-wrapper">
						...
						<div class="ae-icon-list">
							<?php
							$font_awesome_icon_lists = apply_filters('pragyan_fontawesome_icon_list', array());
							?>
							<ul>
								<?php
		...
		function update($new_instance, $old_instance)
		{
			...
			foreach ($widget_fields as $field_key => $field) {
				...
				$instance[$field['name']] = Pragyan_Widget_Validation::instance()->sanitize($new_field_value, $field);
